<?php
  ...
  $game_id = $game_name = $game_qty = $curret_game_qty = "";
  $game_id_err = $game_name_err = $game_qty_err = "";
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        else
        {
            ...
            $sql = "SELECT game_name, stock_qty FROM games WHERE game_name = '".$_POST["game_name"]."' && game_ID = '".$_POST["game_id"]."'";
            ...
            if($result = mysqli_query($link, $sql))
            {
                ...
                if($rowcount == 1)
                {
                        ...
                        //mysqli_stmt_fetch($getArray);
                      //  $getArray = $result->mysqli_fetch_array(MYSQLI_BOTH);
                        $curret_game_qty = $getArray["stock_qty"];
        ...
        if(empty($game_name_err) && empty($game_id_err) && empty($game_qty_err))
        {
            
            // Prepare an insert statement
            $sql = "UPDATE games SET stock_qty=? WHERE (game_ID = ? && game_name = ?)";
            if($stmt = mysqli_prepare($link, $sql))
            {
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt, 'sss', $game_qty, $game_id, $game_name);
                // Set parameters
                $game_qty = (trim($_POST["game_qty"]) + $curret_game_qty);
                $game_name = trim($_POST["game_name"]);
                ...
                {
                    echo "Changed stock of the game!";
                }
            }
...
<div class="w3-main" style="margin-left:250px">
  ...
  <div class="w3-container w3-center" id="jeans">
    <p><b><font size="6">Change Game Stock Quantity</font></b></p>
